...
1.傻逼一样的写了一个BUG特多的alert（BUG不想说，不知道有没有人发现），现在应该可以正常使用了

2.菜单栏可以通过active进行权限控制了！
...
| clickClose    | 是否点击自身可以关闭弹出框                  | Boolean | —          | false   |
| ContextHeight | 中间内容的高度                              | String  | —          | 101px   |
...
| ------ | ---------------------------------------------------- |
| center | 会插入到弹出框中间的内容                             |
| footer | 可以编辑底部的内容，不要和**显示icon情况**下一起使用 |
...
| freshIcon | 刷新图标，可以结合自身喜好（各种图标库）自己选择             | String  | —          | —                                               |
...
| active         | 用于控制权限时使用          | Boolean | true,false | true    |
